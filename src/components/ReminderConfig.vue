...
<template>
  <div class="reminder-config-wrapper">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div v-for="model in models" :key="model.id"
        ...
        <div
          ...
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
              ...
              <Icon :icon="model.expanded ? 'chevron-up' : 'chevron-down'"
        ...
        <transition enter-active-class="transition-all duration-300 ease-out" enter-from-class="opacity-0 max-h-0"
          ...
          <div v-if="model.expanded" class="p-6 bg-linear-to-br from-slate-50 to-blue-50/30">
            <!-- Empty State -->
            <div v-if="model.reminders.length === 0" class="text-center py-16">
              <div
                class="w-20 h-20 bg-linear-to-br from-slate-100 to-slate-200 rounded-2xl flex items-center justify-center mx-auto mb-4">
                <Icon icon="bell" class="w-10 h-10 text-slate-400" />
              </div>
            ...
            <div v-else class="space-y-4">
              <div v-for="reminder in model.reminders" :key="reminder.id"
                ...
                <div class="flex items-start gap-4">
                  <div class="flex-1 space-y-5">
                    ...
                    <div>
                      ...
                      <div class="relative">
                        <input type="number" min="0" :value="reminder.days"
                          ...
                          placeholder="0" @input="updateReminderDays(model.id, reminder.id, $event.target.value)">
                    ...
                    <div class="flex items-center gap-3 p-4 bg-slate-50 rounded-lg border border-slate-200">
                      <input type="checkbox" :id="`daily-${reminder.id}`" :checked="reminder.isDaily"
                      ...
                      <label :for="`daily-${reminder.id}`" class="flex-1 cursor-pointer">
                    ...
                    <div>
                      ...
                      <div class="grid grid-cols-2 gap-3">
                        <button v-for="option in sendTypeOptions" :key="option.value" :class="[
                          ...
                          <Icon :icon="option.value === 'email' ? 'envelope' : 'bell'" class="w-4 h-4" />
                          {{ option.label }}
                          <Icon v-if="(reminder.sendTypes || []).includes(option.value)" icon="check-circle"
                    ...
                    <div>
                      ...
                      <button
                        ...
                        <div class="flex items-center gap-3">
                          <div
                            ...
                            <Icon icon="users" class="w-5 h-5 text-blue-600" />
                          </div>
                          <div class="text-left">
                      ...
                      <!-- Selected Recipients -->
                      <div v-if="(reminder.recipients || []).length > 0"
                        class="mt-3 p-4 bg-slate-50 rounded-lg border border-slate-200">
                        <div class="flex flex-wrap gap-2">
                          <span v-for="recipient in reminder.recipients" :key="recipient"
                  ...
                  <button
                    ...
                    @click="deleteReminder(model.id, reminder.id)" title="Delete reminder">
    ...
    <!-- Recipient Modal -->
    <Modal v-model="showRecipientModal" size="md" :show-close="false">
      <!-- Modal Header -->
      <div class="bg-linear-to-r from-blue-500 to-indigo-600 p-6 text-white">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            <div class="w-12 h-12 bg-white/20 rounded-lg flex items-center justify-center">
              <Icon icon="users" class="w-6 h-6" />
            </div>
            <div>
          ...
          <button class="p-2 hover:bg-white/20 rounded-lg transition-colors" @click="showRecipientModal = false">
      ...
      <div class="p-6 max-h-96 overflow-y-auto">
        <div v-if="availableRecipients.length === 0" class="text-center py-8">
          <Icon icon="users" class="w-12 h-12 text-slate-300 mx-auto mb-3" />
          <p class="text-slate-500 text-sm">No recipients available</p>
        </div>
        <div v-else class="space-y-2">
          <label v-for="recipient in availableRecipients" :key="recipient" :class="[
            ...
            <div class="relative shrink-0">
              <input type="checkbox" :checked="currentReminder?.recipients.includes(recipient)"
                class="w-5 h-5 text-blue-600 rounded border-2 border-slate-300 focus:ring-4 focus:ring-blue-100 transition-all cursor-pointer"
            ...
            <Icon v-if="currentReminder?.recipients.includes(recipient)" icon="check-circle"
