  ...
  <header :class="cn(
    ...
    <div class="flex items-center justify-between h-14 sm:h-16 px-3 sm:px-4 md:px-6">
      ...
      <div class="flex items-center gap-2 sm:gap-4 min-w-0 flex-1">
        ...
        <div v-if="companyLogo && showHeaderLogo" class="shrink-0 hidden sm:block">
          <img :src="companyLogo" alt="Company logo"
            class="w-8 h-8 sm:w-10 sm:h-10 object-contain rounded-lg shadow-sm">
        </div>

        <!-- Divider -->
        <div v-if="companyLogo && showOrganisationInfo && currentOrganisation"
            ...
            <div v-if="showOrganisationDropdown"
              ...
              <div class="p-3 sm:p-4 border-b border-gray-200 bg-linear-to-r from-blue-50 to-indigo-50">
                <h3 class="text-sm font-bold text-gray-900 flex items-center gap-2">
                  <Icon icon="list" class="w-4 h-4 text-blue-600" />
              ...
              <div class="py-2 max-h-80 overflow-y-auto">
                <button v-for="org in organisations" :key="org.org_id || org.organisation_name" :class="cn(
                  ...
                  <div v-if="getOrgLogoUrl(org)" class="shrink-0 mr-2 sm:mr-3">
                    <img :src="getOrgLogoUrl(org)" :alt="`${org.organisation_name} logo`"
                  ...
                  <div v-else
                    ...
                    <Icon icon="building" class="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-500" />
                  </div>
                  <div class="flex-1 text-left min-w-0">
                    <p class="font-semibold truncate text-sm">
                      {{ org.organisation_name }}
                    </p>
                    <p v-if="org.type" class="text-xs text-gray-500 truncate">
                      {{ org.type.name }}
                    </p>
                  </div>
                  <Icon v-if="org.org_id === currentOrganisation?.org_id" icon="check-circle"
        ...
        <nav v-if="showBreadcrumb" class="hidden lg:flex items-center space-x-2 text-sm truncate">
          <span class="text-gray-500 truncate">{{ currentSection }}</span>
          <Icon icon="chevron-right" class="w-4 h-4 text-gray-400 shrink-0" />
      ...
      <div class="flex items-center space-x-1 sm:space-x-2 md:space-x-3">
        ...
        <div v-if="showSearch && (!isMobile || showMobileSearch)" class="relative">
          <div class="absolute inset-y-0 left-0 pl-2 sm:pl-3 flex items-center pointer-events-none">
            <Icon icon="search" class="w-4 h-4 text-gray-400" />
          </div>
          ...
          <div v-if="showNotificationsDropdown"
            ...
            <div class="max-h-[60vh] sm:max-h-96 overflow-y-auto">
              <div v-if="notifications.length === 0" class="p-6 sm:p-8 text-center">
                <Icon icon="bell" class="w-10 h-10 sm:w-12 sm:h-12 mx-auto text-gray-300 mb-2" />
                ...
                <div class="flex items-start space-x-2 sm:space-x-3">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <div class="flex items-start justify-between gap-2">
                      ...
                      <!-- Unread indicator -->
                      <div v-if="!notification.read" class="shrink-0 w-2 h-2 mt-1 bg-blue-500 rounded-full" />
                    </div>

                    <div v-if="notification.source" class="flex items-center gap-1 sm:gap-1.5 mt-1.5 flex-wrap">
                      <span
                        class="inline-flex items-center px-1.5 sm:px-2 py-0.5 rounded text-[10px] sm:text-xs font-semibold bg-indigo-100 text-indigo-800 border border-indigo-200">
                      ...
                      <span
                        class="inline-flex items-center px-1.5 sm:px-2 py-0.5 rounded text-[10px] sm:text-xs font-medium bg-gray-100 text-gray-700 border border-gray-200">
            ...
            <div v-if="notifications.length > 0" class="p-2 sm:p-3 text-center border-t border-gray-200 bg-gray-50">
              <button class="text-xs sm:text-sm font-semibold text-blue-600 hover:text-blue-700 transition-colors"
        ...
        <div class="relative">
          <button
            ...
            <div v-if="showUserDetails" class="hidden md:block text-left max-w-40 truncate">
            ...
            <Icon icon="chevron-down" class="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-400" />
          </button>
