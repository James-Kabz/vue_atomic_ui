<template>
  <div class="bg-white border-b border-gray-200">
    ...
    <div :class="filtersClasses">
      ...
      <div v-for="filter in selectFilters" :key="filter.key" class="min-w-36">
        <div class="relative">
          <Select :model-value="filter.value" :class="selectClasses"
      ...
      <button v-if="showFilters && (dateFilters.length > 0 || hasAdvancedFilters)" :class="advancedFiltersToggleClasses"
      ...
      <div class="flex items-center gap-3 ml-auto">
        ...
        <div v-if="showAdd && isAddButtonVisible">
          ...
          <Button v-if="hasAddButton" :variant="addButton.variant || 'success'" :size="addButton.size || 'lg'"
            :disabled="isAddButtonDisabled || !hasAddButtonPermission" :title="addButton.tooltip"
            @click="handleAddButtonClick">
            <Icon v-if="addButton.icon" :icon="addButton.icon" class="w-4 h-4 mr-2" />
            {{ addButton.label || 'Add' }}
          </Button>

          <!-- Add Button Slot (fallback) -->
          <slot v-else name="add">
            <Button variant="success" size="lg" @click="$emit('add')">
    ...
    <div v-if="showFilters && showAdvancedFilters" :class="advancedFiltersContainerClasses">
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div v-for="numberFilter in numberFilters" :key="numberFilter.key" class="space-y-3">
          <div class="flex items-center justify-between">
            <label :class="dateFilterLabelClasses">
              {{ numberFilter.label }}
            </label>
          ...
          <div class="flex items-center gap-3">
            <div class="relative flex-1">
              <input type="number" :model-value="numberFilter.min" :class="dateInputClasses"
                :placeholder="`Min ${numberFilter.label}`" :step="numberFilter.step || 1"
                @input="updateNumberFilter(numberFilter.key, 'min', $event.target.value)">
            </div>
            <span :class="dateRangeSeparatorClasses">to</span>
            <div class="relative flex-1">
              <input type="number" :model-value="numberFilter.max" :class="dateInputClasses"
                :placeholder="`Max ${numberFilter.label}`" :step="numberFilter.step || 1"
                @input="updateNumberFilter(numberFilter.key, 'max', $event.target.value)">
            </div>
            <button v-if="hasNumberFilterValues(numberFilter)" :class="clearDateFilterButtonClasses"
              title="Clear this filter" @click="clearNumberFilter(numberFilter.key)">
              <Icon icon="xmark" class="w-4 h-4" />
            </button>
          </div>
        </div>
        ...
        <div v-for="multiFilter in multiSelectFilters" :key="multiFilter.key" class="space-y-3">
          <div class="flex items-center justify-between">
            <label :class="dateFilterLabelClasses">
              {{ multiFilter.label }}
            </label>
          ...
          <div class="relative">
            <Select :model-value="''" :class="selectClasses"
              @change="toggleMultiSelectOption(multiFilter.key, $event.target.value)">
              <option value="">
                Select {{ multiFilter.label }}
              </option>
              <option v-for="option in multiFilter.options" :key="option.value" :value="option.value">
                {{ option.label }}
              </option>
            </Select>
            <Icon icon="chevron-down"
          ...
          <div v-if="multiFilter.selected.length > 0" class="flex flex-wrap gap-2 mt-2">
            <span v-for="selectedValue in multiFilter.selected" :key="selectedValue"
  ...
  <!-- File Upload Modal -->
  <div v-if="showFileUpload && isFileUploadModalOpen"
    ...
    <div class="bg-white rounded-xl shadow-2xl max-w-2xl w-full p-6">
      <div class="flex items-center justify-between mb-4">
        ...
        <button class="text-gray-400 hover:text-gray-600 p-2 hover:bg-gray-100 rounded-full transition-all"
          @click="closeFileUploadModal">
          <Icon icon="xmark" class="w-5 h-5" />
        </button>
      </div>

      <FileUpload :multiple="fileUploadMultiple" :accept="fileUploadAccept" :max-size="fileUploadMaxSize"
        :variant="fileUploadVariant" @files-selected="handleFilesSelected" @file-removed="handleFileRemoved" />

      <div class="flex justify-end gap-3 mt-6">
        <Button variant="default" @click="closeFileUploadModal">
          Close
        </Button>
      </div>
    </div>
